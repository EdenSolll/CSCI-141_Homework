...
def roomiest(list_of_boxes, sorted_dict):
    ...
    for item, weight in sorted_dict.items():
        ...
        if current_roomiest_box is not None:
            ...
            packed_items[item] = weight
    remaining_items = {item: weight for item, weight in sorted_dict.items() if item not in packed_items}
...
def tightest_fit(list_of_boxes, sorted_dict):
    ...
    for item, weight in sorted_dict.items():
        ...
        for box in list_of_boxes:
            if box.space_remaining >= weight:
                if box.space_remaining < current_tightest_space or current_tightest_box is None:
                    current_tightest_space = box.space_remaining
